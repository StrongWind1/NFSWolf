...
| [F-5.8: AUTH_NONE Metadata Leak](F-5.8-auth-none-metadata-leak.md) | AUTH_NONE GETATTR may still succeed on the root handle obtained via MOUNT |
| [F-5.1: Export List Enumeration](F-5.1-export-list-enumeration.md) | EXPORT reveals all paths; AUTH_TOOWEAK confirms which ones are Kerberos-only |
| [F-3.8: RPC-with-TLS](F-3.8-rpc-with-tls.md) | TLS transport does not affect AUTH_TOOWEAK -- it operates at a different layer |
