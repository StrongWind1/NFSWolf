...
| | *(F-3.6 was reclassified as an NFSv4 SECINFO finding; see [NFSv4 findings](nfsv4.md). UDP MOUNT handle theft via source-address spoofing remains a real risk on L2-adjacent networks but is no longer a numbered finding.)* | | |
...
MNTPROC_DUMP returns all active mount entries (client hostname/IP and export path), revealing which machines are NFS clients and what they have mounted, providing reconnaissance for credential theft and lateral movement. nfswolf retrieves DUMP from both v1 and v3 via `dump_clients_v1` and `dump_clients` in `src/proto/mount.rs`. See the [MOUNT Protocol DUMP procedure](../../nfs/protocols/mount.md#procedure-2-dump-list-active-mounts) for details.
...
The handoff gap, where MOUNT issues a handle and NFS operates on it without back-checking authorization, is the root cause of F-2.7 (bearer tokens), F-2.5 (stale handles), F-2.8 (sibling lateral), and UDP handle theft via mountd's unverified source addresses. No fix exists within the protocol; NFSv4 eliminates MOUNT but introduces its own traversal issues (F-2.11, F-2.12). See the [MOUNT Protocol](../../nfs/protocols/mount.md) reference for the full architectural analysis.

### MOUNT port discovery

The MOUNT daemon does not run on a well-known port. Its port is registered in the portmapper (program 100005) and must be looked up via PMAPPROC_GETPORT or PMAPPROC_DUMP. When the portmapper is firewalled, the MOUNT port must be guessed or scanned. Common mount ports include 20048 (NFS utils default), 892, and random high ports. nfswolf's `--probe-port` flag supports scanning arbitrary ports for mountd, and `--skip-mountd` bypasses MOUNT entirely when the handle is already known.
...
NFSv4 does not use the MOUNT protocol. File handles are obtained via PUTROOTFH (pseudo-FS root) or LOOKUP from a known directory. This eliminates F-5.1 (export enumeration via MNTPROC_EXPORT), UDP handle theft via MNT, and the auth_flavors leak. However, any server that exposes NFSv3 or NFSv2 alongside v4 still runs a MOUNT daemon, and all MOUNT findings apply to that daemon regardless of whether the attacker ultimately operates over v4. The `scan` subcommand reports MOUNT availability even when the target is primarily a v4 server.
