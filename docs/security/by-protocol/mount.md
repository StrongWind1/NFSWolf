...
The handoff gap, where MOUNT issues a handle and NFS operates on it without back-checking authorization, is the root cause of F-2.7 (bearer tokens), F-2.5 (stale handles), F-2.8 (sibling lateral), and F-3.6 (UDP theft). No fix exists within the protocol; NFSv4 eliminates MOUNT but introduces its own traversal issues (F-2.11, F-2.12). See the [MOUNT Protocol](../../nfs/protocols/mount.md) reference for the full architectural analysis.
