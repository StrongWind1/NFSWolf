...
The `nfs-v4` crate can represent all 37 NFSv4.0 operations (ops 3-39) plus ILLEGAL (op 10044) as `ArgOp` variants, plus 4 NFSv4.1 operations (EXCHANGE_ID op 42, GETDEVICEINFO op 47, GETDEVICELIST op 48, SECINFO_NO_NAME op 52). The `CompoundBuilder` has typed convenience methods for 13+ operations including SETCLIENTID, SECINFO_NO_NAME, EXCHANGE_ID, GETDEVICEINFO, GETDEVICELIST, and OPEN (open_read). The 9 operations below are the core set used by the analyzer and shell for recon and read-only access. The remaining operations are representable on the wire but not yet wired into the binary's attack modules.
...
**Implementation**: All operations use manual `Pack`/`Unpack` implementations in `crates/nfs-v4/src/wire.rs` (re-exported as `types` via `src/proto/nfs4/mod.rs`). The `Nfs4Status` enum carries 25 named variants (Ok + 24 error codes including `BadHandle`, `WrongSec`, `Moved`, and `BadXdr`) plus an `Unknown(u32)` catch-all (26 total), and implements `Display` with RFC 7530 error names (e.g., `NFS4ERR_STALE`). Classification predicates `is_permission_denied()`, `is_stale()`, and `is_not_found()` are available. `Nfs4Error<E>` (in `crates/nfs-v4/src/client.rs`) mirrors `Nfs2Error<E>` / `Nfs3Fault<E>` with three variants: `Rpc(E)`, `Status(Nfs4Status)`, and `MissingResult`. The COMPOUND response is `CompoundRes` with fields `status: u32`, `tag: String`, and `results: Vec<ResOp>`. Each `ResOp` carries `op_code: u32`, `status: u32`, and `data: ResOpData`. SECINFO / SECINFO_NO_NAME responses decode into `Vec<SecInfoEntry>`, where each `SecInfoEntry` carries the flavor number and, for RPCSEC_GSS (flavor 6), the GSS mechanism OID, QOP, and service level. The crate also supports NFSv4.1 operations: `EXCHANGE_ID` (op 42, unauthenticated vendor/version fingerprinting), `SECINFO_NO_NAME` (op 52, security flavors without needing a filename), `GETDEVICEINFO` (op 47, pNFS data-server addresses), and `GETDEVICELIST` (op 48, backend storage mapping). NFSv4.2 security labels are supported via `FATTR4_SEC_LABEL` and `SecLabel4` (RFC 7862).

### NFSv4 client variants

nfswolf has two NFSv4 clients for different use cases:

**`Nfs4Client`** (pool-backed, `src/proto/nfs4/compound.rs`): Used by the analyzer for SECINFO probes and pseudo-FS mapping, and by the interactive v4 shell (`--nfs-version 4`). Shares the connection pool and circuit breaker with v3 calls via `PooledTransport`.
...
**`Nfs4DirectClient`** (direct TCP, `src/proto/nfs4/compound.rs`): Used by the scanner's v4 probe (`probe_nfs4()`). Connects directly to port 2049 without the connection pool. The v4 shell previously used this client but now runs over `PooledTransport` via `Nfs4Client` (see above).
