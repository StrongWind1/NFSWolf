...
| Tool | Language | Status | Description |
|------|----------|--------|-------------|
| [nfscli](https://github.com/claesmnyberg/nfscli) | C | Active (2026) | Standalone NFS v2/v3 client binary by Claes M Nyberg with an FTP-like command-line interface, built for red-team portability with zero dependencies on host NFS packages. Features auto-UID credential cycling, export escape via LOOKUP `..` traversal, file handle harvesting, cross-export pivot, and IPv4 source address spoofing. Presented at 44Con London 2024. No security analysis, no FUSE mount, no NFSv4 support, no structured reporting. |
