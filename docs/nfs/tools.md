...
| Tool | Language | Status | Description |
|------|----------|--------|-------------|
| [nfscli](https://github.com/hvs-consulting/nfscli) | C | Active (2026) | Standalone NFS v2/v3 client binary with an FTP-like command-line interface, built for red-team portability with zero dependencies on host NFS packages. Features auto-UID credential cycling, export escape via LOOKUP `..` traversal, file handle harvesting, cross-export pivot, and IPv4 source address spoofing. Presented at 44Con London 2024. No security analysis, no FUSE mount, no NFSv4 support, no structured reporting. |
| [nfsshell](https://github.com/NetDirect/nfsshell) | C | Active (2025) | The original NFS security tool, first written by Leendert van Doorn in the early 1990s. Provides user-level interactive access to NFS servers over UDP or TCP, with source routing, raw handle injection, `mknod` for device node creation, and privileged port binding. Updated for NFSv3 by NetDirect. No security analysis, no auto-UID escalation, no export escape construction, no NFSv4. |
...
| [nfs4-rs](https://github.com/NetAppLabs/nfs4p1-rs) | Rust | v4.1 | Minimal NFSv4.1 client library and CLI binary. Early-stage development with limited feature coverage. Used as a type reference for NFSv4.1 COMPOUND ops during nfswolf's v4 crate development. Uses serde-xdr (incompatible codec), so nfswolf adapted the types rather than importing. |
...
| [nfsclient (Go)](https://github.com/pschou/nfsclient) | Go | v3 | Simple Go NFS client with ls, upload, download, rm, mkdir, and rmdir capabilities. Allows specifying UID/GID credentials on the command line. |
