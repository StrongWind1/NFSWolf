...
- **Named USER entries** disclose UIDs that have access to the file. Combined with UID spoofing ([F-1.1](../findings/identity/F-1.1-uid-gid-spoofing.md)), an attacker knows exactly which UID to claim in AUTH_SYS credentials.
- **Named GROUP entries** disclose GIDs with access paths not visible in the file's owning group. These feed auxiliary group injection ([F-1.3](../findings/identity/F-1.3-auxiliary-group-injection.md)).
...
!!! warning "No authentication required"
    NFS_ACL uses AUTH_SYS credentials, which the server trusts without verification. An attacker with network access to port 2049 and a valid file handle can call GETACL with `uid=0` (or any UID) and retrieve ACLs for any file on the export. Combined with READDIRPLUS handle harvesting ([F-5.2](../findings/info-disclosure/F-5.2-readdirplus-handle-harvesting.md)), this enables bulk ACL enumeration across an entire export.
...
!!! info "Related findings"
    - [F-5.14: POSIX ACL Entries Expose Access Beyond Mode Bits](../findings/info-disclosure/index.md#f-514-posix-acl-entries-expose-access-beyond-mode-bits): the primary finding for NFS_ACL enumeration
    - [F-1.1: UID/GID Spoofing](../findings/identity/F-1.1-uid-gid-spoofing.md): ACL-discovered UIDs are direct spoofing targets
    - [F-1.3: Auxiliary Group Injection](../findings/identity/F-1.3-auxiliary-group-injection.md): ACL-discovered GIDs are injection targets
    - [F-5.2: READDIRPLUS Handle Harvesting](../findings/info-disclosure/F-5.2-readdirplus-handle-harvesting.md): provides the file handles needed for GETACL calls
