...
- **Which NFS versions are available**, enabling version downgrade attacks ([F-1.6](../findings/identity/F-1.6-nfsv2-downgrade.md))
- **The mountd port**, required to call MNT and obtain file handles
- **Whether NIS is running**, enabling NIS credential extraction ([F-5.3](../findings/info-disclosure/F-5.3-nis-credential-extraction.md))
...
!!! warning "CALLIT attack surface"
    **Amplification**: A small CALLIT request can trigger a large response from the target program, amplified through the portmapper. Combined with UDP source-address spoofing, this enables DDoS reflection attacks with factors of 7x-28x ([F-3.2](../findings/network/F-3.2-portmapper-amplification.md)).

    **Firewall bypass**: CALLIT can reach programs on non-standard ports that are not directly accessible to the attacker. The portmapper forwards the call to `localhost`, bypassing firewall rules that only block external access to the target port ([F-3.5](../findings/network/F-3.5-portmapper-tunnel-bypass.md)).
...
CALLIT enables two distinct attacks: DDoS amplification via UDP source spoofing ([F-3.2](../findings/network/F-3.2-portmapper-amplification.md)), and firewall bypass by proxying calls through localhost ([F-3.5](../findings/network/F-3.5-portmapper-tunnel-bypass.md)).

### GETTIME undermines AUTH_DH

AUTH_DH's timestamp-based authentication collapses when the attacker can query the server's exact clock ([F-3.7](../findings/network/F-3.7-auth-dh-obsolete.md)). GETTIME is the enabler.
...
!!! warning "Blocking port 111 is necessary but incomplete"
    ...
    A complete defense also firewalls port 2049 and the mountd port to trusted source addresses. See the [Firewall Configuration](../defense/configure/firewall.md) guide.
...
!!! info "Related findings"
    - [F-5.4: RPC Service Enumeration](../findings/info-disclosure/F-5.4-rpc-service-enumeration.md) -- DUMP leaks the full service map
    - [F-3.2: Portmapper UDP Amplification](../findings/network/F-3.2-portmapper-amplification.md) -- CALLIT/DUMP for DDoS reflection
    - [F-3.5: Portmapper Tunnel Bypass](../findings/network/F-3.5-portmapper-tunnel-bypass.md) -- CALLIT as a firewall bypass proxy
    - [F-3.7: AUTH_DH Obsolete](../findings/network/F-3.7-auth-dh-obsolete.md) -- GETTIME enables clock-based attacks
    - [F-5.3: NIS Credential Extraction](../findings/info-disclosure/F-5.3-nis-credential-extraction.md) -- DUMP reveals NIS presence
