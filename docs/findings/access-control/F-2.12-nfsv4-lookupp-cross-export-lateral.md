...
2. **Enforce Kerberos (`sec=krb5p`).** This prevents uid/gid spoofing and provides per-user authentication. Cross-export LOOKUP still works but the attacker cannot impersonate other users.
