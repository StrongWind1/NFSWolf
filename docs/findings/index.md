...
!!! abstract "Stateless architecture"
    ...
    The MOUNT protocol is an advisory directory service, not a security gate. The NFS daemon never calls back to MOUNT to verify that a client was authorized, and unmounting does not invalidate file handles. See the [MOUNT protocol](../protocols/mount.md) reference for the full architectural analysis.
...
The findings connect into a directed attack flow. Each stage feeds the next with handles, credentials, or filesystem access.

```mermaid
graph LR
    A["Recon<br>F-5.1, F-5.4, F-5.5"] --> B["Access<br>F-1.1, F-2.7, F-2.9"]
    B --> C["Escape<br>F-2.1, F-2.4, F-2.11"]
    C --> D["Privesc<br>F-4.1, F-4.2, F-4.3"]
    C --> E["Lateral<br>F-2.8, F-2.12"]
    D --> F["Exfil<br>F-5.2, F-5.3"]
    E --> F
    ...
    style E fill:#e52592,color:#fff
    style F fill:#12b5cb,color:#fff
```

| Stage | What happens | Key findings | nfswolf subcommand |
|-------|-------------|-------------|-------------------|
| **Recon** | Enumerate exports, RPC services, pseudo-FS structure, auth flavors | F-5.1, F-5.4, F-5.5 | `scan`, `analyze` |
| **Access** | Forge AUTH_SYS credentials or present a bearer-token handle directly | F-1.1, F-2.7, F-2.9 | `shell`, `mount`, `uid-spray` |
| **Escape** | Break out of the export subdirectory to the filesystem root | F-2.1, F-2.4, F-2.11 | `escape`, `shell escape-root` |
| **Privesc** | Plant SUID binaries or device nodes on writable no_root_squash exports | F-4.1, F-4.2, F-4.3 | `shell`, `mount --allow-write` |
| **Lateral** | Reach sibling exports on the same filesystem via handle reuse or LOOKUPP | F-2.8, F-2.12 | `shell` (cd to sibling) |
| **Exfil** | Harvest handles, read sensitive files, extract NIS credentials | F-5.2, F-5.3 | `shell`, `mount` |

Each stage feeds the next: recon discovers the export topology and auth requirements, access provides a foothold, escape expands the scope from one directory to the full filesystem, and the remaining stages leverage that expanded access for privilege escalation, lateral movement, and data extraction.

For the full narrative walkthrough with example commands at each stage, see the [Attack Chain](attack-chain.md) page.
