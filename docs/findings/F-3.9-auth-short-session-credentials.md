...
- **On Linux targets**: None. AUTH_SHORT is dead code. The finding serves as a server fingerprinting signal -- advertising flavor 2 indicates a non-Linux NFS implementation.
...
|---------|-------------|
| [F-1.1: UID/GID Spoofing](F-1.1-uid-gid-spoofing.md) | AUTH_SYS spoofing is the baseline; AUTH_SHORT is an abbreviated form of the same trust model |
| [F-1.5: Credential Replay](F-1.5-credential-replay.md) | AUTH_SHORT tokens are replayable credentials, a subset of the general credential replay finding |
