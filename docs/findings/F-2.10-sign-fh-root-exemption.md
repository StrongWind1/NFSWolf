...
1. **Enable `subtree_check`** alongside `sign_fh` -- limits LOOKUP traversal to the exported subtree even if the root handle is constructed:
   ```
   /srv/share  client(rw,sign_fh,subtree_check)
   ```

2. **Firewall NFS port 2049** to the same IP ranges as the MOUNT ACL -- prevents direct NFS calls from unauthorized clients
