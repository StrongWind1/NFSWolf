...
```bash
# nfs_analyze automated check
nfs_analyze target --check-no-root-squash
```

This creates a temporary directory as UID 0 and checks if it succeeded. If it does, `no_root_squash` is confirmed.
