...
/// the request never got a protocol answer, while [`Denied`](Self::Denied)
...
pub enum MountError<E> {
    ...
    Rpc(E),
    /// The mount server denied the request with this status code.
    Denied(mountstat3),
}
...
impl<E> From<E> for MountError<E> {
    fn from(e: E) -> Self {
        Self::Rpc(e)
    }
}
