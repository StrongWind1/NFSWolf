...
The crate carries no connection policy: no pooling, no retries, no timeouts, no pacing. It is generic over `nfswolf_rpc::RpcTransport`, so a caller supplies whichever of those it wants. `DirectTransport` gives you a working client over one socket with none of it.
