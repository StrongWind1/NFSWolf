...
#![allow(missing_copy_implementations, reason = "Copy is derived on the wire types whose callers benefit from it; demanding it exhaustively cascades through every containing struct without improving the API")]
...
impl<T: RpcTransport> MountV1Client<T> {
    ...
    pub async fn null(&self) -> Result<(), T::Error> {
        let _: nfswolf_xdr::Void = self.raw_call(proc::MOUNTPROC_NULL, &nfswolf_xdr::Void).await?;
        Ok(())
    }
    ...
    pub async fn umnt(&self, path: dirpath<'_>) -> Result<(), T::Error> {
        let _: nfswolf_xdr::Void = self.raw_call(proc::MOUNTPROC_UMNT, &path).await?;
        Ok(())
    }
    ...
    pub async fn export(&self) -> Result<exports<'static, 'static>, T::Error> {
        self.raw_call(proc::MOUNTPROC_EXPORT, &nfswolf_xdr::Void).await
    }
