...
mod tests {
    ...
    // Golden vector tests
    //
    // Each test starts from a hand-constructed byte slice (the "golden vector")
    // and verifies both decoding and re-encoding produce exact matches. This
    // catches self-consistently wrong codecs where encode and decode are broken
    // in the same direction.
    // =========================================================================

    /// Golden vector: RPC CALL for NFSv3 GETATTR (RFC 5531 sec 9).
    ///
    /// xid=0x42, CALL, rpcvers=2, prog=100003, vers=3, proc=1, AUTH_NULL cred+verf.
    #[test]
    fn golden_rpc_call_nfsv3_getattr() {
        #[rustfmt::skip]
        const GOLDEN: [u8; 40] = [
            // xid
            0x00, 0x00, 0x00, 0x42,
            ...
            // proc = 1 (GETATTR)
            0x00, 0x00, 0x00, 0x01,
            // cred: AUTH_NULL flavor + empty body
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00,
            // verf: AUTH_NULL flavor + empty body
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00,
        ];
        ...
        let (msg, consumed) = rpc_msg::unpack(&mut cursor).expect("unpack rpc_msg");
        assert_eq!(consumed, GOLDEN.len());
        assert_eq!(msg.xid, 0x42);
        match msg.body {
            msg_body::CALL(ref call) => {
                assert_eq!(call.rpcvers, RPC_VERSION_2);
                assert_eq!(call.prog, 100_003);
                assert_eq!(call.vers, 3);
                assert_eq!(call.proc, 1);
                assert_eq!(call.cred.flavor, auth_flavor::AUTH_NULL);
                assert!(call.cred.body.as_ref().is_empty());
                assert_eq!(call.verf.flavor, auth_flavor::AUTH_NULL);
                assert!(call.verf.body.as_ref().is_empty());
            },
        ...
        let _ = msg.pack(&mut repacked).expect("pack");
        assert_eq!(repacked, GOLDEN);
    }

    /// Golden vector: RPC REPLY MSG_ACCEPTED SUCCESS (RFC 5531 sec 13.1).
    ///
    /// xid=0x42, REPLY, MSG_ACCEPTED, AUTH_NULL verf, SUCCESS.
    #[test]
    fn golden_rpc_reply_accepted_success() {
        #[rustfmt::skip]
        const GOLDEN: [u8; 24] = [
            // xid
            0x00, 0x00, 0x00, 0x42,
        ...
        let mut cursor = Cursor::new(&GOLDEN[..]);
        let (msg, consumed) = rpc_msg::unpack(&mut cursor).expect("unpack");
        assert_eq!(consumed, GOLDEN.len());
        assert_eq!(msg.xid, 0x42);
        match msg.body {
            msg_body::REPLY(reply_body::MSG_ACCEPTED(ref reply)) => {
                assert_eq!(reply.verf.flavor, auth_flavor::AUTH_NULL);
                assert!(matches!(reply.reply_data, accept_stat_data::SUCCESS));
            },
            _ => panic!("expected MSG_ACCEPTED SUCCESS"),
        }
    ...
    /// Golden vector: auth_unix body (RFC 5531 sec 14).
    ///
    /// stamp=1, machinename="host", uid=1000, gid=1000, gids=[1000].
    #[test]
    fn golden_auth_unix_body() {
        #[rustfmt::skip]
        const GOLDEN: [u8; 28] = [
            // stamp = 1
            0x00, 0x00, 0x00, 0x01,
            // machinename length = 4
            0x00, 0x00, 0x00, 0x04,
            // machinename "host" (4 bytes, no padding needed)
            0x68, 0x6F, 0x73, 0x74,
            // uid = 1000 (0x3E8)
            0x00, 0x00, 0x03, 0xE8,
            // gid = 1000
        ...
        assert_eq!(consumed, GOLDEN.len());
        assert_eq!(decoded.stamp, 1);
        assert_eq!(decoded.machinename.as_ref(), b"host");
        assert_eq!(decoded.uid, 1000);
        assert_eq!(decoded.gid, 1000);
        assert_eq!(decoded.gids, vec![1000]);

        let mut repacked = Vec::new();
        let _ = decoded.pack(&mut repacked).expect("pack");
        assert_eq!(repacked, GOLDEN);
    }
}
