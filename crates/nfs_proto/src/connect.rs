...
pub struct Nfs3Connection<IO> {
    /// Server this connection was established against.
    pub host: String,
    /// Port the MOUNT service was reached on.
    pub mount_port: u16,
    /// Export path that was mounted.
    pub mount_path: dirpath<'static>,
    /// Client for the MOUNT service, retained so the export can be unmounted.
    pub mount_client: MountClient<IO>,
    /// Successful `MNT` reply: the export's root file handle and the auth
    /// flavors the server will accept for it.
    pub mount_resok: mountres3_ok<'static>,
    /// Port the NFSv3 service was reached on.
    pub nfs3_port: u16,
    ...
    IO: AsyncRead + AsyncWrite + Send,
{
    ...
    pub fn root_nfs_fh3(&self) -> nfs_fh3 {
        nfs_fh3 { data: self.mount_resok.fhandle.0.clone() }
    }
    ...
    S: AsyncRead + AsyncWrite + Send,
{
    ...
    pub fn new(connector: C, host: impl AsRef<str>, mount_path: impl AsRef<str>) -> Self {
        Self {
            ...
            portmapper_port: portmap::PMAP_PORT,
            mount_port: None,
            nfs3_port: None,
    ...
    pub async fn mount(self) -> Result<Nfs3Connection<S>, ConnectError> {
        ...
        let mut mount_client = MountClient::new_with_auth(io, self.credential.clone(), self.verifier.clone());
        ...
        Ok(Nfs3Connection { host: self.host, mount_port, mount_path: self.mount_path, mount_client, mount_resok, nfs3_port, nfs3_client })
    }
    ...
    async fn resolve_ports(&self) -> Result<(u16, u16), ConnectError> {
        ...
        let addr = self.host.parse().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e))?;
        let io = self.connector.connect(SocketAddr::new(addr, self.portmapper_port)).await?;

        let mut portmapper = portmap::PortmapperClient::new(io);
        let mount_port = if let Some(port) = self.mount_port { port } else { portmapper.getport(mount::PROGRAM, mount::VERSION, portmap::IPPROTO_TCP).await? };

        let nfs3_port = if let Some(port) = self.nfs3_port { port } else { portmapper.getport(crate::nfs3::PROGRAM, crate::nfs3::VERSION, portmap::IPPROTO_TCP).await? };
    ...
    async fn connect(&self, port: u16) -> std::io::Result<S> {
        ...
        if self.connect_from_privileged_port { connect_from_privileged_port(&self.connector, socket_addr).await } else { self.connector.connect(socket_addr).await }
    }
}
    ...
    S: AsyncRead + AsyncWrite + Send,
{
    ...
    Err(IoError::other(format!("Failed to connect to mount service from privileged port range ({MIN_PORT}-{MAX_PORT})")))
}
