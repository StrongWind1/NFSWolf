//! File handle analysis integration tests.
//!
//! Tests what can be tested without a lib target: the MemFs server behaviour
//! that the scanner and analyzer modules rely on (handle format, MOUNT response,
//! auth-flavor advertisement). The core fingerprinting/escape logic is covered
//! by the unit tests embedded in `src/engine/file_handle.rs`.
#![allow(
